...
def plot_evolution_over_time(min_costs_over_time, max_costs_over_time, average_costs_over_time):
    ...
    plt.title('M1 Sequential vs Fitness')
    plt.legend()
    plt.show()
...
for x in range(number_solutions):
    ...
    for s in best_solutions:
        if max < M1(binary_to_float(s)):
            max = M1(binary_to_float(s))
            final = s
    final_best.append(final)
